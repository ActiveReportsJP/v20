...
Friend NotInheritable Class DataLayer
	' IEnumerableデータオブジェクトを返します。
	Public Shared Function LoadData() As IList(Of Year)
		Dim years = New List(Of Year)()

		Dim connStr = My.Resources.ConnectionString
		Dim conn = New SQLiteConnection(connStr)
		conn.Open()
		Dim cmd = New SQLiteCommand("SELECT Movie.YearReleased, Movie.MovieID, Movie.Title, Movie.MPAA FROM Movie ORDER BY Movie.YearReleased", conn)
