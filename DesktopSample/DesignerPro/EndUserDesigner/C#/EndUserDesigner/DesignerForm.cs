...
namespace ActiveReports.Samples.Designer
{
	internal partial class DesignerForm : Form
	{
		...
		private const string ReportPartsDirectoryKey = "Designer.Controls.ReportPartsDirectory";
		private const string ShowReportsLibraryKey = "Designer.Controls.ShowReportsLibrary";
		...
		private void TryLoadReport(string fileName)
		{
			...
			catch
			{
				...
				if (string.IsNullOrEmpty(_reportName)
					|| !new FileInfo(_reportName).Exists
		...
		private void SetReportName(string reportName)
		{
			...
			Text = string.Format(Resources.TitleFormat,	string.IsNullOrEmpty(reportName) ? GetDefaultReportName(_reportType) : Path.GetFileName(reportName), IsDirty ? Resources.DirtySign : string.Empty, Resources.DesignerFormTitle);
		}
		...
		private string GetDefaultReportName(DesignerReportType reportType)
		{
			switch (reportType)
			{
				...
				case DesignerReportType.Page:
					return Resources.DefaultReportNamePage;
				case DesignerReportType.RdlMultiSection:
					return Resources.DefaultReportNameRdlx;
				case DesignerReportType.RdlDashboard:
					return GetIsMaster() ? Resources.DefaultReportNameRdlxMaster : Resources.DefaultReportNameRdlDashboard;
			}
